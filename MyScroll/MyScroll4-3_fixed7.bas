...
sub InitMySprite()
    ...
    ' Sprite transparency index (NextReg $4B). For 8-bit patterns the transparent index is a full byte (default $E3).
    WriteNextReg($4B, $E3)   ' 8-bit sprite transparency index (matches default after reset)
...
sub UpdateBouncingSprite()
    ...
    UpdateSprite(ux, uy, SPR_ID, SPR_PATTERN, $10, 0)   ' 0 => 8-bit anchor (256 bytes per pattern)
end sub
